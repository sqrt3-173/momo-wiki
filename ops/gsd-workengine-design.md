...
2. **Planning flow** — `new-project` + `plan-phase`. **✅ MACHINERY DONE (2026-07-02)** — awaiting first live run (gated on `confirm_roadmap`).
   ...
   - ✅ **Docs → wiki (2026-07-02).** `wiki/projects/skip-bin/` = PROJECT.md + REQUIREMENTS.md (14 reqs recovered verbatim from the roadmapper transcript) + ROADMAP.md. ROADMAP is a **rendered view**: `render_roadmap(slug)` SQL function (`worksystem/005-gates-render.sql`) → `psql -t -A -c "SELECT render_roadmap('skip-bin')" > ROADMAP.md`. Never hand-edit; re-render after any phase/plan change.
   - ✅ **Gates wired (2026-07-02).** `tasks.gate` column + `create_gate(slug, gate, title, …)` + `gate_blocking(slug, gate)` (same file). GSD gate = human task (assignee eli, priority 4) surfaced via `human_queue`. `confirm_roadmap` gate live for skip-bin (task 5); planning proceeds only when it's done.
   - ✅ **plan-phase machinery (2026-07-02).** GSD sources pulled verbatim → `worksystem/gsd-source/` (agents: phase-researcher/planner/plan-checker; templates: project/requirements/roadmap/research/spec — closes the research.md ingestion gap). Adapted prompts: `worksystem/agents/{phase-researcher,planner,plan-checker}.md` (methodology faithful; substrate = JSON→DB + wiki docs; fidelity trade-offs flagged inside each file's source-adaptation notes... see below). `materialize_plans(phase_id, jsonb)` (`worksystem/006-materialize-plans.sql`) inserts plans+tasks, maps depends_on numbers→ids — **proven in a rolled-back transaction** (waves, deps {1}→{id}, acceptance criteria, renderer pickup). `claim_next_task()` now excludes plan-attached tasks (they're the executor's checklist, claimed via `claim_next_plan`). Orchestration runbook: `worksystem/plan-phase.md`.
   - ⚠️ Known fidelity gaps (flagged by the adaptation pass, revisit on demand): planner drops GSD's `type: tdd` plan type (no column; TDD survives as test-first task ordering), STRIDE threat-model + package-legitimacy severity register, MVP/walking-skeleton + gap-closure modes; checker drops REVIEWS.md/PATTERNS.md/VALIDATION.md dimensions (no substrate artifacts yet); researcher drops Nyquist validation architecture.
   - ✅ **PROVEN AT SCALE (2026-07-02/03):** the full pipeline ran SIX consecutive phases on the work-dashboard build — researcher (1×), planner (6×), adversarial checker (6×, every verdict approved-with-warnings, several catching real defects incl. an engine bug), materialise, orchestrator-executed waves, per-phase verification. v1 shipped: 14/14 REQs, 98 tests. Execution model = DL-07 (orchestrator executes serially; subagent executors need a guard decision first — that's the stage-3 conversation).
   - ⬜ Next: stage 3 proper (parallel executor fan-out — needs Eli's guard call), verifier agent (goal-backward), ship step. Skip-bin is parked (Eli's call — gate open).
   - ⚠️ Deviation, deliberate: work-dashboard phase 1 planned WITHOUT a ui-phase/UI-SPEC (machinery not built yet; phase is a low-visual foundation slice — scaffold + one grid). **Build the ui-phase step before planning phase 2** (drill-down/wave lanes — properly visual).
