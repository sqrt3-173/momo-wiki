...
- **Guard** deployed: `~/nunu/ops/nunu-guard.py` + `~/nunu/.claude/settings.local.json` (PreToolUse hook). Auto-approves marketing work (reads, writes in `/Users/nunu`, dev/git, web, Discord, Claude Design, own-wiki push); hard-blocks money/system/destructive/RCE/install/publish; NO interactive-ask (Yana's non-technical) — unsafe = blocked (flag Eli); Eli override = `#ELI_OK`. **Currently UNLOCKED** (self-protecting vs NUNU's own tools) — lock immutable after real-work testing: `sudo chown root:wheel` + `sudo chmod 444` + `sudo chflags uchg` on `nunu-guard.py` + `settings.local.json`.
  - **Decision LOGGING added to BOTH guards (2026-07-01):** every allow/block writes to `ops/logs/guard.log` (`<ts> ALLOW|BLOCK <tool> :: <reason>`). **Tune-then-lock plan:** after Yana's real use, read NUNU's log → add whatever got wrongly BLOCKED to the allow-list, strip whatever's never used (least-privilege) → *then* lock. Likely first over-blocks to expect (from the fail-closed allow-list): `unzip` (design archive), image tools (`sips`/ImageMagick), Canva "generate" MCP verb, `zip`. Guard update = edit template in `ops/nunu-templates/` → stage in `nunu-wiki/_deploy/` → NUNU pulls + `cp` into place (hook re-runs fresh each call, no restart). MOMO's guard is locked → unlock→cp→relock (sudo) to edit.
